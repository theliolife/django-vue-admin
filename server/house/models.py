...
class House(models.Model):
    ...
    utime = models.DateTimeField('更新时间', default=datetime.now)
    ...
    def dashboard():
        ...
        operate_time = datetime.now().strftime("%Y-%m-%d")
        cursor.execute(f"select operate_time,count(*) as total from sp_house group by operate_time order by operate_time desc limit 30")
    ...
    def list(operate_time = ''):
        ...
        cursor.execute(f"select * from sp_house where operate_time = '{operate_time}'")
        list = House.dict_fetch_all(cursor)
